...
class KPCNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.sattn1 = MultiHeadAttention(feature_dim=256, num_heads=4)
...
# variable to disable the self attenion module
global selfattn
selfattn = True
...
class KPFCNN(nn.Module):
    ...
    def __init__(self, config, sym=True):
        ...
        self.encoder_skips = []
        self.sa = SelfAttention(n_channels=32)
        # self.sa = SimpleSelfAttention(n_in = 32, ks=1, sym=sym)
    ...
    def forward(self, batch):
        ...
        if selfattn: 
            # self-attention network
            x = self.sa(x)
            # print("self-attention module active")

        features = F.normalize(x, p=2, dim=-1)
        # features = torch.cat((features, batch['points'][0]), 1)
